...
if (!class_exists("NicePayPalButtonLite")) {
  ...
  class NicePayPalButtonLite {
    ...
    function printAdminPage() {
      ...
      if (isset($_POST['update_nicePayPalBUttonLiteSettings'])) {
        ...
        if (isset($_POST['currency_code'])) {
          ...
          $niceOptions['currency_code'] = sanitize_text_field($_POST['currency_code']);

        }

        if (isset($_POST['country_code'])) {

          $niceOptions['country_code'] = sanitize_text_field($_POST['country_code']);

        }

        if (isset($_POST['paypal_testmode'])) {

          $niceOptions['paypal_testmode'] = sanitize_text_field($_POST['paypal_testmode']);

        }

        if (isset($_POST['button_path'])) {

          $niceOptions['button_path'] = sanitize_text_field($_POST['button_path']);

        }

        if (isset($_POST['default_btnsize'])) {

          $niceOptions['default_btnsize'] = sanitize_text_field($_POST['default_btnsize']);

        }

        if (isset($_POST['open_window'])) {

          $niceOptions['open_window'] = sanitize_text_field($_POST['open_window']);

        }

        if (isset($_POST['paypal_url'])) {

          $niceOptions['paypal_url'] = sanitize_text_field($_POST['paypal_url']);

        }

        if (isset($_POST['paypal_testurl'])) {

          $niceOptions['paypal_testurl'] = sanitize_text_field($_POST['paypal_testurl']);

        }

        if (isset($_POST['paypal_email'])) {

          $niceOptions['paypal_email'] = sanitize_text_field($_POST['paypal_email']);

        }

        if (isset($_POST['paypal_testemail'])) {

          $niceOptions['paypal_testemail'] = sanitize_text_field($_POST['paypal_testemail']);

        }

        if (isset($_POST['paypal_partner'])) {

          $niceOptions['paypal_partner'] = sanitize_text_field($_POST['paypal_partner']);
      ...
      <div class=wrap>
        <form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
          ...
          <h3>PayPal Partner Identifier</h3>
          <p>Select on or off to allow our PayPal Partner identifier to be sent with submissions to PayPal. The identifier does not affect your transactions at all and is invisible to site visitors. When you allow the identifier, you help to support future efforts to provide quality plugins to the WordPress community.</p>
          <p><label for="paypal_partner0">
            <input type="radio" id="paypal_partner0" name="paypal_partner" value="0" <?php if ($niceOptions['paypal_partner'] == 0) { _e('checked="checked"', "nicePayPalButton"); }?> />off</label>&nbsp;&nbsp;&nbsp;&nbsp;
          <label for="jform_params_paypal_testmode1">
            <input type="radio" id="paypal_partner1" name="paypal_partner" value="1" <?php if ($niceOptions['paypal_partner'] == 1) { _e('checked="checked"', "nicePayPalButton"); }?> />on</label>
          </p>
          <div class="submit">
    ...
    function getNicePayPalButtonLite ( $atts = '' ){
      ...
      $atts['paypal_partner']   = !empty($opts['paypal_partner']) ? $opts['paypal_partner'] : 0;
    ...
    function nicePayPalButtonLiteLiteBuildForm( $a )
    {
      ...
      if ( $a['name'] != '' && $a['command'] != '' && $a['email'] != '' && $a['url'] != '' ){
        ...
        //shipping
        if ( isset($a['shipping']) && $a['shipping'] != '' ){
        ...
        //shipping2
        if ( isset($a['shipping2']) && $a['shipping2'] != '' ){
        ...
        $f .= !empty($a['paypal_partner']) && $a['paypal_partner'] != 0 ? '<input type="hidden" name="bn" value="TriniTronic_SP">' : '';
...
if ( !function_exists("nicePayPalButtonLite_ap") ) {
    ...
    function nicePayPalButtonLite_ap() {
        ...
        if ( function_exists('add_options_page') ) {
          ...
          add_options_page('Nice PayPal Button Lite', 'Nice PayPal Button Lite', 'manage_options', basename(__FILE__), array(&$nice_paypalButtonLite, 'printAdminPage'));
